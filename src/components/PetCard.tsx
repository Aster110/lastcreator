  ...
  return (
    <div className="fixed inset-0 bg-gray-950 flex flex-col px-6 py-16 overflow-y-auto">
      <div className="flex-1 flex flex-col items-center gap-6">
        {/* 图像占位 */}
        <div className="w-48 h-48 rounded-2xl bg-gray-800 flex items-center justify-center text-gray-600 text-5xl">
          🐾
        </div>
        ...
        <div className="w-full space-y-2 text-sm">
          <div className="flex justify-between text-gray-500">
            <span>栖息地</span><span className="text-gray-300">{pet.habitat}</span>
          </div>
          <div className="flex justify-between text-gray-500">
            <span>性格</span><span className="text-gray-300">{pet.personality}</span>
          </div>
          <div className="flex justify-between text-gray-500">
            <span>HP</span><span className="text-gray-300">{pet.hp}</span>
          </div>
        </div>
        ...
        <div className="w-full">
          ...
          <div className="flex flex-wrap gap-2">
            {pet.skills.map(s => (
              <span key={s} className="px-3 py-1 rounded-full bg-gray-800 text-gray-300 text-xs">{s}</span>
            ))}
          </div>
        </div>
